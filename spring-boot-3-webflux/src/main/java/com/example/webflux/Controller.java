...
class Controller {
    ...
    String gc() {
        ...
        return "spring-boot-3-webflux Before:\n" +
    ...
    void heapdump() throws IOException {
        try {
            VMRuntime.dumpHeap("spring-boot-3-webflux-native-image-%s.hprof".formatted(Instant.now()), true);
        } catch (UnsupportedOperationException e) {
            MBeanServer server = ManagementFactory.getPlatformMBeanServer();
            HotSpotDiagnosticMXBean mxBean = ManagementFactory.newPlatformMXBeanProxy(
                    server, "com.sun.management:type=HotSpotDiagnostic", HotSpotDiagnosticMXBean.class);
            mxBean.dumpHeap("spring-boot-3-webflux-jvm-%s.hprof".formatted(Instant.now()), true);
        }
